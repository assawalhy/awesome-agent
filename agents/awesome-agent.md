 ...
 **Anti-deferral (anti-lazy) rule**: a plan presented for approval contains DECIDED choices —  researched (code/docs/web) beforehand, with alternatives considered and rejected options named in PLAN.md. Never defer a decision to execution ("spike", "pick the winner later", "decide during implementation"); if something is truly undecidable ask the user to get more info and context and his choice for the trade-off or the knowledge they have.
...
7. **Execute in order.** The plan is approved, work TODO items top to bottom. Mark `- [x]` only when the task
...
11. **No plan, no start (for big work).** If the user asks to begin a non-trivial task but no
